...
class MovieRecommender:
    ...
    def train(self, verbose=False):
        ...
        self.user_similarity = 1 - pairwise_distances(
            self.train_data_matrix, metric="cosine"
        )
    ...
    def predict_user_based_naive(self, top=5):
        ...
        try:
            ...
            print(
                f"Параметр top={top} выходит за пределы top_similar_ratings.shape[0]={top_similar_ratings.shape[0]}"
            )
            return False
        ...
        try:
            ...
            for i in range(self.n_users):  # для каждого пользователя
                ...
                for j, sim_user_idx in enumerate(similar_users_idx):
                    ...
                    weighted_sum[valid_ratings_mask] += (
                        similarity * normalized_ratings[valid_ratings_mask]
                    )
                ...
                for movie_idx in range(self.n_movies):
                    if similarity_sum[movie_idx] > 0:
                        ...
                        pred[i, movie_idx] = (
                            current_user_mean
                            + weighted_sum[movie_idx] / similarity_sum[movie_idx]
                        )
                    else:
    ...
    def precision_recall_at_k(self, pred, actual, k=10, threshold=3.5):
        ...
        for i in range(self.n_users):
            ...
            # Получаем предсказания только для фильмов, которые есть в тесте
            test_movies = set(np.where(actual[i] > 0)[0])
        
            # Предсказания только для тестовых фильмов
            test_predictions = {idx: pred[i][idx] for idx in test_movies}

            # Предсказанные top-K фильмов
            predicted_indices = sorted(
                test_predictions.keys(), 
                key=lambda x: test_predictions[x], 
                reverse=True
            )[:k]
    ...
    def recommend_movies(
        self, user_id, pred, n=10, include_ratings=False, verbose=False
    ):
        """
    ...
    def compare_predictions_with_actual(self, user_id, pred, n=20):
        ...
        user_ratings = MovieRating.objects.filter(user_id=user_id).select_related(
            "movie"
        )
        ...
        for item in comparison_data[:n]:  # Показываем топ-N расхождений
            ...
            diff_str = f"{diff:.2f}"
            print(
                f"{item['movie_id']:<4} {item['title']:<40} {item['actual_rating']:<10} {item['predicted_rating']:<14} {diff_str:<8}"
            )
        ...
        if comparison_data:
            ...
            print(
                f"Differences (<1) count: {good_predictions}/{len(comparison_data)} ({good_predictions / len(comparison_data) * 100:.1f}%)"
            )
...
def main():
    user_id = User.objects.get(username="user_197_2").id
    ...
    ratings = (
        MovieRating.objects.filter(user_id=user_id)
    ...
    precision, recall = recommender.precision_recall_at_k(
        user_pred, recommender.test_data_matrix
    )
    print(f"UserToUser Precision = {precision:.2f}. Recall = {recall:.2f}")
    print(
        f"UserToUser F1-score: {recommender.f1_score(user_pred, recommender.test_data_matrix):.2f}"
    )
    print("Рекоммендации пользователя")
    recommender.recommend_movies(
        user_id, user_pred, n=10, include_ratings=True, verbose=True
    )
