...
class HybridRecommender:
    ...
    def recommend_hybrid(self, user_id, n=10, cf_weight=0.5, cb_weight=0.5, verbose=False):
        """
        ...
        if cf_weight > 0 and self.is_trained:
            if user_id in self.collaborative.user_to_index:
                ...
                if not isinstance(pred, bool):
                    cf_movies = self.collaborative.recommend_movies(user_id, pred, n=n*3)
        ...
        for movie, score in cf_movies_with_scores:
            movie_scores[movie.id] = movie_scores.get(movie.id, 0) + score * cf_weight_norm
        
        for movie, score in cb_movies_with_scores:
            movie_scores[movie.id] = movie_scores.get(movie.id, 0) + score * cb_weight_norm
        
        # Исключаем уже просмотренные
        excluded_ids = set(
            list(Favorite.objects.filter(user=user).values_list('movie_id', flat=True)) +
            list(MovieRating.objects.filter(user=user).values_list('movie_id', flat=True))
        )
        
        # Сортируем и получаем топ-N
        sorted_movies = sorted(
            [(movie_id, score) for movie_id, score in movie_scores.items() if movie_id not in excluded_ids], 
            key=lambda x: x[1], 
            reverse=True
        )
        ...
        for movie_id, score in sorted_movies[:n]:
            try:
                ...
                if verbose:
                    print(f"{movie.id:2d}. {movie.movie_name:40} | Гибридный score: {score:.3f}")
...
def main():
    ...
    ratings = MovieRating.objects.filter(user=user).select_related('movie').order_by('-user_rating')
    ...
    if user_id in hybrid.collaborative.user_to_index:
        ...
        if not isinstance(pred, bool):
            cf_recs = hybrid.collaborative.recommend_movies(user_id, pred, n=5, verbose=True)
    ...
    hybrid_recs_balanced = hybrid.recommend_hybrid(user_id, n=5, cf_weight=0.5, cb_weight=0.5, verbose=True)
