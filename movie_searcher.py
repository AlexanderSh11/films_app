    ...
    if _MODEL is None:
        try:
            model_path = os.path.join(
                os.path.dirname(__file__), "fine_tuned_model\\fine_tuned_rubert_tiny2"
            )
...
def build_embeddings_cache():
    ...
    embeddings = _MODEL.encode(
        movie_texts, convert_to_numpy=True, show_progress_bar=True
    )
...
def search_movies(query, top_k=5, movie_ids=None):
    ...
    if movie_ids is not None:
        ...
        distances, indices = temp_index.search(
            query_embedding, min(top_k, len(valid_movie_ids))
        )
    ...
    movies = Movie.objects.filter(id__in=result_ids).prefetch_related(
...
def evaluate_model(model, movies_data, test_queries):
    ...
    for q in test_queries:
        ...
        for i, (movie, score) in enumerate(zip(retrieved_movies[:5], scores[:5]), 1):
            ...
            print(
                f"  {i}. [{relevant}] {movie.movie_name} ({movie.year}) - {score:.4f}"
            )
...
if __name__ == "__main__":
    ...
    test_single_query()
    movies_data = list(
        Movie.objects.prefetch_related("genre", "director", "country").all()
    )
