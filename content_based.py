...
class MovieRecommender:
    ...
    def get_user_preferences(self):
        ...
        user_ratings = (
            MovieRating.objects.filter(user=self.user)
            .select_related("movie")
            .prefetch_related("movie__genre", "movie__director", "movie__country")
        )
        ...
        if (
            user_ratings.count() + favorites.count() < 10
        ):  # минимум 10 избранных/оцененных фильмов для анализа
    ...
    def recommend_movies(self, n=5):
        ...
        # print("Веса стран:\n", country_prefs, sep="")
        # print()
        if genre_prefs is None or director_prefs is None or country_prefs is None:
            return []

        # получаем все фильмы, которые пользователь еще не добавлял в избранное
        excluded_ids = Favorite.objects.filter(user=self.user).values_list(
            "movie_id", flat=True
        )
        all_movies = Movie.objects.exclude(id__in=excluded_ids).prefetch_related(
